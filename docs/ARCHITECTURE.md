...
- `codex-app-server`
  ...
  - for requests with a non-empty `systemPrompt`, it can internally warm or reuse a persistent seed thread and fork ephemeral child threads from it
...
`codex-app-server` exists to reduce repeated Codex startup cost while keeping the public HTTP API lean.
...
- `POST /v1/generate` with `model: "codex-app-server"` and a non-empty `systemPrompt` derives an internal seed identity from normalized `systemPrompt + providerModel`
...
- the adapter keeps lightweight in-memory metadata for each internal seed entry: seed id, thread id, provider model, and timestamps
- repeated requests with the same normalized `systemPrompt + providerModel` reuse the same seed automatically
